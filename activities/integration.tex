...
\item For each slope field make sketches for different solutions for
  ...
  \begin{eqnarray*}
    ...
    y(0) & = & -3.
  \end{eqnarray*}
  For each slope field describe the general behavior and state the
  long term solutions if they exist.
...
\item Make a sketch of the slope field for the equation
  ...
  Sketch solutions to the differential equation for different initial
  conditions. Does the solution grow or decay? Are there circumstances
  where the solution will approach a specific value?

  \vfill

\item Can you make up a differential equation that will always
  approach the value of $y=5$ if the initial condition is greater than
  one but will always decrease otherwise? What is the equation? Is the
  equation unique?
