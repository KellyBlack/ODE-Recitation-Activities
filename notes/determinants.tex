...
\begin{frame}
  ...
  \only<2->{Suppose that $x$,  $y$, $u$, $v$, and $c$ are real numbers.}
  \begin{eqnarray*}
    \only<3->{\vecTwo{x}{y} & \in & \mathbb{R}^2} \\
    \only<4->{\vecTwo{x}{y} + \vecTwo{u}{v} & = & \vecTwo{x+u}{y+v}} \\
    \only<5->{c \vecTwo{x}{y} & = & \vecTwo{cx}{cy}} \\
    \only<6->{\vec{0} & = & \vecTwo{0}{0}} \\
    \only<7->{\vecTwo{x}{y} + \vecTwo{-x}{-y} & = & \vec{0}}
  \end{eqnarray*}
...
\begin{frame}
  ...
  \begin{itemize}
  \item<2-> $f(t)+g(t)$ is continuous.
  \item<3-> $c f(t)$ is continuous if $c$ is a real number.
  \item<4-> $h(t)=0$ is the zero vector.
  \item<5-> For any function
    \begin{eqnarray*}
      f(t) + (-1)f(t) & = & (1 + (-1))f(t), \\
      & = & 0 f(t), \\
      & = & 0.
