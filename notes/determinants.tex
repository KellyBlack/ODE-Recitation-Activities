...
\title{Determinants}
\subtitle{Math 232 - Do Solutions Exist?}
\date{22 Oct 2014}
...
\begin{frame}
  ...
  The \textbf{\redText{minor}}, $M_{ij}$ of a matrix is found by \textit{removing} row
  $i$ and column $j$.
...
\begin{frame}
  ...
  \end{eqnarray*}
  This is the determinant by \redText{expansion along the
    $i^{\mathrm{th}}$ row}. It is the same for any $i$ where $1\leq i
  \leq n$.
...
\begin{frame}
  ...
  The vector space, V, is a \redText{set of objects.}
...
\begin{frame}
  ...
  \begin{eqnarray*}
    \only<1->{\vecTwo{x}{y},~\vecTwo{u}{v} & \in & \mathbb{R}^2} \\
...
\begin{frame}
  \frametitle{Example - The Set of Straight Lines}
