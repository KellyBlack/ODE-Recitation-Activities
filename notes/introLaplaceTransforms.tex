...
\begin{frame}
  ...
  The Laplace transform of a sum:
  \begin{eqnarray*}
  ...
  The Laplace transform of a constant times a  function:

  Suppose $a$ is a constant, then
  \begin{eqnarray*}
...
\begin{frame}
  ...
  \uncover<2->
  {
    ...
    \begin{eqnarray*}
      \begin{array}{rclclcl}
      \laplace{f} & = & \redText{\laplace{3}}  & + & \blueText{\laplace{5t}}  & + &  \fuchsiaText{\laplace{-8t^2}}, \\
                  & = & \redText{3\laplace{1}} & + &  \blueText{5\laplace{t}} & - & \fuchsiaText{8\laplace{t^2}}, \\
                  & = & \redText{\frac{3}{s}}  & + &  \blueText{\frac{5}{s^2}} & - & \fuchsiaText{\frac{16}{s^3}}.
                \end{array}
    \end{eqnarray*}
...
\begin{frame}
  ...
  \begin{eqnarray*}
    ...
    \only<2->{
      ...
      \Rightarrow \redText{\laplace{e^{at}}} & \redText{=} & \redText{\frac{1}{s-a}}.
    }
  \end{eqnarray*}
...
\begin{frame}
  ...
  \begin{columns}
    ...
    \column{.9\textwidth}
    \begin{eqnarray*}
      \laplace{\blueText{e^{at}} f(t)} & = & \int^\infty_0 ~\blueText{e^{at}} f(t) \redText{e^{-st}} ~ dt, \\
      \uncover<2->
      {
        & = & \int^\infty_0 f(t) \blueText{e^{-(s-a)t}} ~ dt, \\
      }
      \uncover<3->
      {
        & = & \laplace{f}(\blueText{s-a}).
      }
    \end{eqnarray*}
  \end{columns}
...
\begin{frame}
  \frametitle{Clicker Quiz}
   ...
   \ifnum\value{clickerQuiz}=1{%
     ...
     Are there any more identities?

     \begin{tabular}{ll}
       A: & Yes \\ [12pt]
       B: & No, that would be lame, \\ [12pt]
       C: & Golly, jeepers I hope so!
     \end{tabular}
